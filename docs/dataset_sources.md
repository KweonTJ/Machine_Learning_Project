...
| 5 | Vehicle Maintenance Telemetry Data | 차량 텔레메트리 기반 점검 추천 데모 | https://www.kaggle.com/datasets/tejalaveti2306/vehicle-maintenance-telemetry-data | Kaggle 공개 차량 fleet telemetry 데이터셋. 파일명은 `synthetic_telemetry_data.csv`이지만, 본 프로젝트에서 임의 생성한 데이터는 아니다. Kaggle 계정과 API 토큰이 필요하다. |
